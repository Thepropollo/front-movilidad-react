...
import { Wrench, ShieldAlert, CheckCircle2, AlertTriangle, Trash2, Search, Calendar, User, Settings } from 'lucide-react';
...
const WorkshopPanelPage: React.FC = () => {
  ...
  const [mechanics, setMechanics] = useState<Array<{ id: number; first_name: string; last_name: string }>>([]);
  ...
  const [maintenanceType, setMaintenanceType] = useState<'preventivo' | 'correctivo' | 'cambio_aceite'>('correctivo');
  ...
  const [selectedSupplies, setSelectedSupplies] = useState<Array<{ supply: SupplyItem; quantity: number }>>([]);
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.message || 'Error al iniciar la orden de trabajo.');
    } finally {
  ...
  const handleCloseOrder = async () => {
    ...
    for (const item of selectedSupplies) {
      if (item.quantity > item.supply.current_stock) {
        setErrorMsg(`Cantidad solicitada para ${item.supply.supply_name} supera el stock disponible (${item.supply.current_stock}).`);
        return;
      }
    }
    ...
    try {
      ...
      setSuccessMsg('Unidad reparada con éxito. Vehículo liberado para circulación.');
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.message || 'Error al cerrar la orden de trabajo.');
    } finally {
    ...
    <div className="glass-panel wide-container mx-auto" style={{ textAlign: 'left' }}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-muted mt-1">Gestión técnica del libro de novedades, órdenes de reparación e insumos de la flota.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="glass-panel p-6 bg-white/50 flex flex-col">
            ...
            <p className="text-muted text-xs mb-6">Alertas reportadas por el personal de garita o conductores durante el checklist digital.</p>
            ...
            {issues.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center border border-dashed border-gray-200 rounded-2xl py-12 text-center bg-gray-50/50">
                ...
                <p className="font-semibold text-gray-500">Sin novedades pendientes</p>
                <p className="text-muted text-xs mt-1">Todos los vehículos se encuentran en buen estado operativo.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-4 overflow-y-auto max-h-[500px] pr-1">
                {issues.map(issue => (
                  <div key={issue.id} className="p-4 rounded-xl border border-red-100 bg-red-50/30 flex flex-col gap-3 shadow-xs">
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <span className="font-bold text-red-800 font-mono text-sm mr-2">{issue.vehicle.plate}</span>
                        <span className="text-xs text-primary font-medium">{issue.vehicle.brand} {issue.vehicle.model}</span>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                        issue.status === 'pendiente' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'
                      }`}>
                        {issue.status}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs text-gray-500 border-t border-gray-100/50 pt-2 gap-2">
                      <div className="flex items-center gap-1.5">
                        <User size={13} />
                        <span>Reportado por: {issue.reporting_driver.first_name} {issue.reporting_driver.last_name}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <Calendar size={13} />
                        <span>{new Date(issue.breakdown_date).toLocaleDateString()}</span>
                      </div>
                    </div>
          ...
          <div className="glass-panel p-6 bg-white/50 flex flex-col">
            ...
            <p className="text-muted text-xs mb-6">Unidades actualmente bajo mantenimiento correctivo, preventivo o de lubricación.</p>

            {workOrders.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center border border-dashed border-gray-200 rounded-2xl py-12 text-center bg-gray-50/50">
                <Wrench className="text-gray-300 mb-2" size={40} />
                <p className="font-semibold text-gray-500">Taller vacío</p>
                <p className="text-muted text-xs mt-1">No hay órdenes de reparación activas en curso.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-4 overflow-y-auto max-h-[500px] pr-1">
                {workOrders.map(order => (
                  <div key={order.id} className="p-4 rounded-xl border border-gray-200 bg-white flex flex-col gap-3 shadow-xs">
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <span className="font-bold text-primary font-mono text-sm mr-2">{order.vehicle.plate}</span>
                        <span className="text-xs text-primary font-medium">{order.vehicle.brand} {order.vehicle.model}</span>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                    ...
                    <div className="text-sm text-gray-700 bg-gray-50 p-2.5 rounded-lg font-mono text-xs">
                      <p className="font-bold text-gray-600 mb-1">TRABAJOS REQUERIDOS:</p>
                      <p className="italic text-gray-800">"{order.work_details}"</p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs text-gray-500 pt-1 gap-2 border-t border-gray-100">
                      <div className="flex items-center gap-1.5">
                        <User size={13} />
                        <span>Mecánico: {order.responsible_mechanic?.first_name || 'No asignado'}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <Calendar size={13} />
                        <span>Entrada: {new Date(order.entry_date).toLocaleDateString()}</span>
                      </div>
                    </div>
          ...
          <form className="flex flex-col gap-4">
            <div className="bg-gray-50 p-3 rounded-lg text-sm grid grid-cols-2 gap-2 border border-gray-100">
              <div>
                ...
                <p className="font-semibold text-primary">{selectedIssue.vehicle.brand} {selectedIssue.vehicle.model}</p>
                <p className="text-secondary-brand font-mono font-bold text-xs">{selectedIssue.vehicle.plate}</p>
              </div>
              <div>
                <p className="text-muted text-xs">Kilometraje de Entrada</p>
                <p className="font-semibold text-primary">{selectedIssue.vehicle.current_mileage} km</p>
              </div>
            </div>

            <div className="form-group">
              <label className="form-label" htmlFor="maintenance-type-select">Tipo de Mantenimiento</label>
              <select
              ...
              >
                ...
                <option value="cambio_aceite">Cambio de Aceite (Lubricación + Filtros)</option>
              </select>
            </div>

            <div className="form-group">
              <label className="form-label" htmlFor="mechanic-select">Mecánico Responsable</label>
              <select
                id="mechanic-select"
                className="form-input"
                value={mechanicId}
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="work-details-textarea">Especificaciones de Trabajo</label>
              <textarea
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-gray-50 p-4 rounded-xl text-sm border border-gray-150 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <p className="font-semibold text-primary">{selectedOrder.vehicle.brand} {selectedOrder.vehicle.model}</p>
                <p className="text-secondary-brand font-mono font-bold text-xs">{selectedOrder.vehicle.plate}</p>
              </div>
              <div>
                <p className="text-muted text-xs">Mecánico</p>
                <p className="font-semibold text-primary">{selectedOrder.responsible_mechanic?.first_name} {selectedOrder.responsible_mechanic?.last_name}</p>
              </div>
              <div>
                <p className="text-muted text-xs">Tipo de Reparación</p>
                <p className="font-semibold text-primary capitalize">{selectedOrder.maintenance_type.replace('_', ' ')}</p>
              </div>
            </div>
                ...
                {searchResults.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-50 max-h-48 overflow-y-auto">
                    {searchResults.map(supply => (
                      ...
                      >
                        <span className="font-medium text-primary">{supply.supply_name}</span>
                        <span className="text-xs text-muted">Stock: {supply.current_stock} {supply.measurement_unit}</span>
                      </button>
                    ))}
                  </div>
                )}
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {selectedSupplies.map(item => (
                        <tr key={item.supply.id} className="hover:bg-gray-50/40">
                          <td className="p-3">
                            <span className="font-semibold text-primary text-sm">{item.supply.supply_name}</span>
                            <span className="text-xs text-muted block">{item.supply.measurement_unit}</span>
                          </td>
                          <td className="p-3">
                            <input
                              type="number"
                              min="1"
                              value={item.quantity}
                              onChange={(e) => handleQtyChange(item.supply.id, Number(e.target.value))}
                              className="form-input py-1 px-2 text-center text-sm w-20"
                            />
                          </td>
                          <td className="p-3 text-xs font-semibold">
                            <span className={item.quantity > item.supply.current_stock ? 'text-danger' : 'text-success'}>
                              {item.supply.current_stock} {item.supply.measurement_unit}
                            </span>
                          </td>
