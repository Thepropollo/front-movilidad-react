...
import { FileText, ClipboardList, UploadCloud, CheckCircle, AlertTriangle, Calculator, FileCheck, Landmark, DollarSign, Calendar, Clock } from 'lucide-react';
...
const TeacherLiquidationPage: React.FC = () => {
  ...
  const [selectedSheet, setSelectedSheet] = useState<RouteSheetSummary | null>(null);
  ...
  const handleSelectSheet = async (sheet: RouteSheetSummary) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.message || 'Error al calcular los haberes de la comisión.');
    } finally {
  ...
  const handleSubmitLiquidation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Factura de respaldo y liquidación enviadas correctamente a auditoría.');
      setSelectedSheet(null);
      setCalc(null);
      setUploadedFileName(null);
      setUploadedFileUrl(null);
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.message || 'Error al liquidar la comisión.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="glass-panel wide-container mx-auto" style={{ textAlign: 'left' }}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-muted mt-1">Carga comprobantes de viáticos y combustibles para auditar y cerrar la hoja de ruta de la comisión.</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted">Cargando tus comisiones pendientes de cierre...</p>
        </div>
      ) : (
                ...
                <div className="flex flex-col gap-3 max-h-96 overflow-y-auto pr-1">
                  {sheets.map(sheet => (
                    ...
                    >
                      ...
                      <p className="font-bold text-primary text-sm">Destino: {sheet.request.destination}</p>
                      <p className="text-muted text-[11px] mt-1">Chofer: {sheet.driver.user.first_name} {sheet.driver.user.last_name}</p>
                      <p className="text-muted text-[11px]">Vehículo: {sheet.vehicle.brand} ({sheet.vehicle.plate})</p>
                    </button>
                  ))}
                </div>
              )}
                ...
                <div className="glass-panel p-6 bg-white/50 relative">
                  ...
                  {calculating ? (
                    <div className="flex flex-col items-center justify-center py-6">
                      <div className="spinner mb-2" style={{ width: '30px', height: '30px' }}></div>
                      <p className="text-muted text-xs">Computando viáticos y horas extras...</p>
                    </div>
                  ) : calc ? (
                    <div className="flex flex-col gap-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs bg-white border border-gray-100 p-4 rounded-xl shadow-xs">
                        <div>
                          ...
                          <p className="font-bold text-primary">{calc.departure_real}</p>
                        </div>
                        <div>
                          <p className="text-gray-400 font-medium flex items-center gap-1.5 mb-1">
                          ...
                          <p className="font-bold text-primary">{calc.arrival_real}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="flex justify-between items-center p-3.5 bg-white border border-gray-100 rounded-2xl shadow-xs">
                          ...
                          <span className="font-mono font-bold text-primary text-base">${calc.allowances_amount.toFixed(2)}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center p-3.5 bg-white border border-gray-100 rounded-2xl shadow-xs">
                          ...
                          <span className="font-mono font-bold text-primary text-base">${calc.overtime_50_amount.toFixed(2)}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center p-3.5 bg-white border border-gray-100 rounded-2xl shadow-xs">
                          ...
                          <span className="font-mono font-bold text-primary text-base">${calc.overtime_100_amount.toFixed(2)}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center p-4 bg-gold/10 border border-gold/25 rounded-2xl mt-2">
                          ...
                          <span className="font-mono font-black text-gold-dark text-lg">${calc.total_payout.toFixed(2)}</span>
                        </div>
                ...
                {calc && (
                  <form onSubmit={handleSubmitLiquidation} className="flex flex-col gap-6">
                    <div className="glass-panel p-6 bg-white/50">
                      ...
                      <p className="text-muted text-xs mb-4">Arrastra y suelta el comprobante de liquidación del viaje para la auditoría de movilidad.</p>
                      ...
                      >
                        ...
                        {uploadingFile ? (
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <p className="text-xs text-muted">Subiendo archivo de respaldo...</p>
                          </div>
                        ) : uploadedFileName ? (
                          <div className="flex flex-col items-center gap-2 text-center">
                            <FileText className="text-success" size={40} />
                            <p className="text-xs font-bold text-success-text">{uploadedFileName}</p>
                            <p className="text-[10px] text-gray-400 font-mono">Respaldo cargado correctamente.</p>
                            <button
                              type="button"
                              onClick={() => {
                                setUploadedFileName(null);
                          ...
                          <label htmlFor="file-upload-input" className="cursor-pointer flex flex-col items-center gap-2">
                            <UploadCloud className="text-gray-400 hover:text-gold transition-colors" size={44} />
                            <p className="text-xs text-gray-500 font-medium">
                              Arrastra tu archivo aquí o <span className="text-gold-dark font-bold hover:underline">selecciona desde tu equipo</span>
                            </p>
                            <p className="text-[10px] text-gray-400">Archivos permitidos: PDF, imágenes JPG/PNG (Max. 5MB)</p>
                          </label>
                        )}
                      </div>
                    </div>
