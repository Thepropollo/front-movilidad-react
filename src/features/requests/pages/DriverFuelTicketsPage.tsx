...
import { Fuel, QrCode, ClipboardCheck, MapPin, CheckCircle, AlertTriangle } from 'lucide-react';
...
const DriverFuelTicketsPage: React.FC = () => {
  ...
  const handleEmitVoucher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.message || 'Error al emitir el vale de combustible.');
    } finally {
      setIssuing(false);
    }
  };
    ...
    <div className="glass-panel wide-container mx-auto" style={{ textAlign: 'left' }}>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted">Cargando vales y registros de combustible...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          {user?.role?.name === 'jefe_transporte' && (
            <div className="xl:col-span-1 flex flex-col gap-6">
              <div className="glass-panel p-6 bg-white/5 dark:bg-slate-800/40">
                ...
                <p className="text-muted text-xs mb-4">Formaliza una hoja de ruta autorizando el cupo de combustible de viaje de ida.</p>

                <form onSubmit={handleEmitVoucher} className="flex flex-col gap-4">
                  <div className="form-group">
                    <label className="form-label" htmlFor="route-sheet-select">Hoja de Ruta</label>
                    <select
                    ...
                    >
                      ...
                      {routeSheets
                        ...
                        .filter(sheet => !orders.some(o => o.route_sheet_id === sheet.id))
                        .map(sheet => (
                          <option key={sheet.id} value={sheet.id}>
                            [{sheet.vehicle.plate}] {sheet.vehicle.brand} - {sheet.request.destination}
                          </option>
                        ))}
                    </select>
                  </div>
                  ...
                  <div className="form-group">
                    <label className="form-label" htmlFor="station-select">Estación de Servicio</label>
                    <select
                      id="station-select"
                      className="form-input"
          ...
          <div className={`${user?.role?.name === 'jefe_transporte' ? 'xl:col-span-2' : 'xl:col-span-3'}`}>
            ...
            {orders.length === 0 ? (
              <div className="border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center bg-gray-50/50">
                ...
                <p className="font-semibold text-gray-500">Sin vales emitidos</p>
                <p className="text-muted text-sm max-w-sm mt-1 mx-auto">No hay vales de combustible registrados para comisiones operativas en este momento.</p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                        ...
                        <div>
                          <div className="flex justify-between items-start gap-2 mb-3">
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
                              isConsumed ? 'bg-gray-100 text-gray-600' : 'bg-gold/15 text-gold-dark border border-gold/20'
                            }`}>
                              {order.order_status === 'emitida' ? 'ACTIVO' : 'DESPACHADO'}
                            </span>
                          ...
                          <h3 className="font-bold text-primary text-base flex items-center gap-1.5">
                            <MapPin size={16} className="text-secondary-brand flex-shrink-0" />
                            {order.station.commercial_name}
                          </h3>
                          <p className="text-muted text-xs mt-1">{order.station.address}</p>

                          <div className="grid grid-cols-2 gap-3 mt-4 text-xs">
                            <div>
                              <p className="text-gray-400">Placa Vehículo</p>
                              <p className="font-bold text-primary font-mono text-sm uppercase">{order.route_sheet.vehicle.plate}</p>
                            </div>
                            <div>
                              <p className="text-gray-400">Tipo Combustible</p>
                              <p className="font-bold text-gold-dark font-mono text-sm capitalize">{order.dispatched_fuel_type}</p>
                            </div>
                            <div className="col-span-2">
                              <p className="text-gray-400">Destino de la Comisión</p>
                              <p className="font-semibold text-primary">{order.route_sheet.request.destination}</p>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="p-6 md:w-44 flex flex-col items-center justify-center bg-slate-50/50 dark:bg-slate-900/30 shrink-0">
                        ...
                        <p className="text-center font-mono font-bold text-xs mt-3 text-primary">{order.order_code}</p>
                      </div>
