...
import { ClipboardList, Car, Gauge, Fuel, CheckCircle, AlertTriangle } from 'lucide-react';
...
const ChecklistDigitalPage: React.FC = () => {
  ...
  const [selectedSheet, setSelectedSheet] = useState<PendingRouteSheet | null>(null);
  
  // Form states
  const [mileage, setMileage] = useState<string>('');
  const [fuelLevel, setFuelLevel] = useState<'1/4' | '1/2' | '3/4' | 'full'>('full');
  const [checklistStates, setChecklistStates] = useState<Record<number, 'BUENO' | 'REGULAR' | 'MALO'>>({});
  ...
  const handleConditionChange = (componentId: number, condition: 'BUENO' | 'REGULAR' | 'MALO') => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const regType: 'salida' | 'llegada' = selectedSheet.trip_status === 'programado' ? 'salida' : 'llegada';

    // Mileage validations
    if (regType === 'salida' && numMileage < selectedSheet.vehicle.current_mileage) {
      setErrorMsg(`El kilometraje de salida no puede ser menor al kilometraje actual del vehículo (${selectedSheet.vehicle.current_mileage} km).`);
      return;
    }

    if (regType === 'llegada' && selectedSheet.initial_mileage && numMileage < selectedSheet.initial_mileage) {
      setErrorMsg(`El kilometraje de llegada no puede ser menor al de salida (${selectedSheet.initial_mileage} km).`);
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.message || 'Error al guardar la inspección.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="glass-panel wide-container mx-auto" style={{ textAlign: 'left' }}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-muted mt-1">Control técnico de vehículos al despacho (salida) e ingreso (llegada) de comisiones.</p>
        </div>
      </div>
      ...
      {successMsg && (
        <div className="success-banner mb-6 flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-success-text">¡Registro Exitoso!</p>
      ...
      {fetching ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted">Cargando hojas de ruta e ítems del checklist...</p>
        </div>
      ) : (
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            <div className="glass-panel p-6 bg-white/50">
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="route-sheet-select">Hoja de Ruta Activa</label>
                <select
                ...
                >
                  ...
                  {routeSheets.map(sheet => (
                    <option key={sheet.id} value={sheet.id}>
                      [{sheet.vehicle.plate}] {sheet.vehicle.brand} {sheet.vehicle.model} - {sheet.request.destination} ({sheet.trip_status === 'programado' ? 'Salida' : 'Llegada'})
                    </option>
                  ))}
                </select>
                {routeSheets.length === 0 && (
                  <p className="text-muted text-xs mt-2">No hay vehículos pendientes de salida o llegada en este momento.</p>
                )}
              </div>
                ...
                <div className="mt-6 border-t border-gray-100 pt-6 flex flex-col gap-4">
                  <div>
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      selectedSheet.trip_status === 'programado' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800'
                    }`}>
                      Inspección de {selectedSheet.trip_status === 'programado' ? 'Salida (Despacho)' : 'Llegada (Recepción)'}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm mt-2">
                    <div>
                      ...
                      <p className="font-semibold text-primary">{selectedSheet.vehicle.brand} {selectedSheet.vehicle.model}</p>
                      <p className="text-secondary-brand font-mono font-bold text-xs">{selectedSheet.vehicle.plate}</p>
                    </div>
                    <div>
                      <p className="text-muted text-xs">Conductor</p>
                      <p className="font-semibold text-primary">{selectedSheet.driver.user.first_name} {selectedSheet.driver.user.last_name}</p>
                      <p className="text-muted text-xs">Licencia: {selectedSheet.driver.contract_type}</p>
                    </div>
                    <div className="col-span-2">
                      <p className="text-muted text-xs">Destino & Motivo</p>
                      <p className="font-semibold text-primary">{selectedSheet.request.destination}</p>
                      <p className="text-muted text-xs italic">"{selectedSheet.request.travel_reason}"</p>
                    </div>
                    <div>
                    ...
                    {selectedSheet.trip_status === 'en_ruta' && (
                      <div>
                        <p className="text-muted text-xs">Registrado al Salir</p>
                        <p className="font-semibold text-primary">{selectedSheet.initial_mileage} km</p>
                      </div>
                    )}
                  </div>
                </div>
                      ...
                      <div key={category} className="border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                        ...
                        <div className="flex flex-col gap-4">
                          {components
                            ...
                            .map((comp) => {
                              const currentCondition = checklistStates[comp.id] || 'BUENO';
                              return (
                                <div 
                                  key={comp.id} 
                                ...
                                >
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      type="button"
                                      onClick={() => handleConditionChange(comp.id, 'BUENO')}
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => handleConditionChange(comp.id, 'REGULAR')}
                                      className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-all ${
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => handleConditionChange(comp.id, 'MALO')}
                                      className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-all ${
                ...
                <div className={`p-6 rounded-2xl border transition-all ${
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                    <div className="flex items-start gap-3">
                      {hasMaloComponent ? (
                        <>
                          ...
                          <div>
                            <p className="font-bold text-danger">Excepción del Taller Detectada</p>
                            <p className="text-xs text-red-700 mt-1">Al marcar elementos defectuosos (MALO), el sistema derivará automáticamente la unidad a estado "En Taller" en el libro de novedades, bloqueando su salida.</p>
                          </div>
                        </>
                      ) : (
                        <>
                          ...
                          <div>
                            <p className="font-bold text-success">Todo se encuentra en orden</p>
                            <p className="text-xs text-green-700 mt-1">El checklist está limpio. El vehículo será autorizado para salir a ruta e iniciar la comisión asignada.</p>
                          </div>
                        </>
                      )}
                    </div>
                    ...
                    <Button
                      ...
                      icon={hasMaloComponent ? <AlertTriangle size={18} /> : <CheckCircle size={18} />}
                      className="whitespace-nowrap px-6 py-3"
                    >
                      {hasMaloComponent ? 'Reportar Novedad y Derivar a Taller' : 'Aprobar Despacho y Salida'}
                    </Button>
                  </div>
                </div>
              </form>
